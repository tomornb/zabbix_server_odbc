...
dm_name="unixODBC 2.3.0 DriverManager"
dm_dir="unixODBC-2.3.0"
dm_package='unixODBC-2.3.0.tar.gz'
...
function check_for_Linux_x86_64 ()
{
    ...
    local proc=$(uname -a|awk '{print$12}');
...
function build
{
    ...
    if [ ${#build_steps[@]} -ne ${#build_msg[@]} ]; then
    ...
    do
        ...
        echo_status_aligned "${build_msg[$i]} " "$status_msg"
...
function approve_download
{
    ...
    echo "Exitting because warning not accepted"
    exit 1
}
